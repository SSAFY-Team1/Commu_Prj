...
서울 관광·문화·축제·숙박·쇼핑 등 지역 정보를 한곳에서 검색하고, 익명 게시판에서 개인 경험을 기록하며, 자연어 챗봇과 시각화 대시보드로 정보를 빠르게 파악할 수 있는 LocalHub MVP를 1일 안에 완성한다.
...
| 구분 | 주역할 | 핵심 담당 |
|---|---|---|---|---|---|
| 팀원 A | PM·공통 UI·배포 | 범위 통제, 라우터, 레이아웃, 반응형, Netlify 배포, 발표 통합 |
| 팀원 B | 데이터·대시보드 | JSON 로더, 필터·검색, Chart.js, 데이터 출처 문서 |
| 팀원 C | 커뮤니티·챗봇 | localStorage CRUD, 비밀번호 검증, Netlify Function, 챗봇 UI |
